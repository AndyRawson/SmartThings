...
metadata {
	...
	definition (name: "Thermostat with Humidity and Auto Time setting", namespace: "minollo", author: "minollo") { // CUSTOMIZATION
		capability "Actuator"
	...
	tiles {
		...
		main "temperature"
			details(["temperature", "mode", "fanMode", "heatLevelDown", "heatingSetpoint", "heatLevelUp", "coolLevelDown", "coolingSetpoint", "coolLevelUp", "heatSliderControl", "heatingSetpoint", "coolSliderControl", "coolingSetpoint", "battery", "humidity", "refresh", "configure"])
        // END CUSTOMIZATION
	}
}
...
def poll() {
	delayBetween([
		...
		zwave.multiInstanceV1.multiInstanceCmdEncap(instance: 2).encapsulate(zwave.sensorMultilevelV3.sensorMultilevelGet()).format() // CT-100/101 Customization
	], 2300)
}
...
def setHeatingSetpoint(Double degrees, Integer delay = 30000) {
	...
	sendEvent(name: "setHeatingSetPoint", value: "set heat temp to $degrees")
...
def setCoolingSetpoint(Double degrees, Integer delay = 30000) {
    ...
    sendEvent(name: "setCoolingSetPoint", value: "set cool temp to $degrees")
	def deviceScale = state.scale ?: 1
	def deviceScaleString = deviceScale == 2 ? "C" : "F"
    def locationScale = getTemperatureScale()
	def p = (state.precision == null) ? 1 : state.precision
...
def switchToMode(nextMode) {
	...
	if (nextMode in modes()) {
		state.lastTriedMode = nextMode
		"$nextMode"()
		sendEvent(name: "SwitchMode", value: "switched to $nextMode mode")
	} else {
		log.debug("no mode method '$nextMode'")
		sendEvent(name: "SwitchMode", value: "cannot switch to $nextMode mode, does not exist")
	}
}
